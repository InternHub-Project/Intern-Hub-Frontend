...
export default function Companies() {
  ...
  // useEffect(() => {
  //   fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.company.topBrands}`)
  //     .then((res) => res.json())
  //     .then((data) => {
  ...
  return (
    <div>
      <Box ta={"center"}>
        <p className={classes.title}>Top companies trust us</p>
      </Box>
      <div className={classes.back}>
        <div className={classes.up}>
          <Marquee speed={80} gradient={true}>
            <div className={classes.image}>
              {imageCompanies.map((item) => (
                <span key={item.id}>
                  <img
                    src={item.image}
                    width={item.width}
                    height={item.height}
                  />
                  {/* <Text fz={20} fw={600}>{item.name}</Text> */}
                </span>
              ))}
            </div>
          </Marquee>
